...
struct semaforos {
		...
		sem_t sem_hornearPanterminado;
		sem_t sem_cortarExtrasterminado;
		sem_t sem_Hamburguesascocinadas;
...
void* cocinarHamburguesa(void *data) {
		...
		//Doy la senial a la siguiente accion (cocinar me habilita armar hamburguesa)

		sem_post(&mydata->semaforos_param.sem_Hamburguesascocinadas);
...
void* hornearPan(void *data) {
	...
	//Llamo a la funcion imprimir le paso el struct y la accion de la funcion
		...
		sem_post(&hornoUnico);
		//Doy la señal a la siguiente accion (hornear me habilita a armar la hamburguesa completa)
        sem_post(&mydata->semaforos_param.sem_hornearPanterminado);
...
void* cortarExtras(void *data) {
	...
	sem_post(&mydata->semaforos_param.sem_cortarExtrasterminado);
    	pthread_exit(NULL);
}
...
void* armarHamburguesa(void *data) {
        ...
        //Creo el puntero para pasarle la referencia de memoria (data) del struct pasado por parametro (la cual es un puntero).
        struct parametro *mydata = data;
		sem_wait(&mydata->semaforos_param.sem_cortarExtrasterminado);
		sem_wait(&mydata->semaforos_param.sem_hornearPanterminado);
		sem_wait(&mydata->semaforos_param.sem_Hamburguesascocinadas);
...
void* ejecutarReceta(void *i){
	...
	pthread_data->semaforos_param.sem_cortarExtrasterminado = sem_cortarExtrasterminado;
	pthread_data->semaforos_param.sem_hornearPanterminado= sem_hornearPanterminado;
	pthread_data->semaforos_param.sem_armarHamburguesa = sem_armarHamburguesa;
	pthread_data->semaforos_param.sem_Hamburguesascocinadas = sem_Hamburguesascocinadas;
	...
	sem_init(&(pthread_data->semaforos_param.sem_cortarExtrasterminado),0,0);
	sem_init(&(pthread_data->semaforos_param.sem_hornearPanterminado),0,0);
	sem_init(&(pthread_data->semaforos_param.sem_armarHamburguesa),0,0);
	sem_init(&(pthread_data->semaforos_param.sem_Hamburguesascocinadas),0,0);
	...
	sem_destroy(&sem_cortarExtrasterminado);
	sem_destroy(&sem_hornearPanterminado);
	sem_destroy(&sem_armarHamburguesa);
	sem_destroy(&sem_Hamburguesascocinadas);
